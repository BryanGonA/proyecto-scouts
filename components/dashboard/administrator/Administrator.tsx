...
export default function PersonalAdministrator() {
          ...
          return res.json().then(data => {
            ...
            let u ={
              ...
              <Link href={`/dashboard/jefe-grupo/personal/${user.id}`}>
                  <a><img className={styles2.icon} src="/img/dashboard/eye.svg" /></a>
              </Link>
              <Link href={`/dashboard/jefe-grupo/personal/editar/${user.id}`}>
                  <a><img className={styles2.icon} src="/img/dashboard/pencil.svg" /></a>
              </Link>
              <Link href={`/dashboard/jefe-grupo/personal/`}>
                  <a><img className={styles2.icon} src="/img/dashboard/trash.svg"  
                 onClick={ () =>{
    ...
    return (
      <div>
             <div>
                <h1 className={`${styles.text_title} p-5`}>Gestión de personal administrativo Grupo Scout Centinelas 113</h1>
            </div>
    {loading ? (
        <div className="row col-12 mt-5 justify-content-center" >
          Cargando Información...
            ...
            <div className={`${styles.text_data} row justify-content-center py-3`}>
               ...
               <button type="button" className={`${styles.boton}`}><a className={`${styles.letra}`} href={`/dashboard/jefe-grupo/personal/agregar/`}>Agregar personal</a></button>      
